...
impl Model {
    ...
    fn load_model(path: &Path) -> io::Result<Self> {
        ...
        let vb = unsafe {
            VarBuilder::from_mmaped_safetensors(&[weights_path], DTYPE, &device)
                .map_err(|e| IoError::other(format!("Failed to load weights: {e}")))?
        };
        let weights = BertModel::load(vb, &config).map_err(|e| {
            IoError::other(format!(
                "Failed to load BertModel from VarBuilder and config: {e}"
            ))
        })?;
    ...
    pub fn from_slices(config: &[u8], tokenizer: &[u8], weights: &[u8]) -> io::Result<Self> {
        ...
        let mut tokenizer = Tokenizer::from_bytes(tokenizer)
            .map_err(|e| IoError::other(format!("Failed to load tokenizer from buffer: {e}")))?;
        let pp = PaddingParams {
            strategy: tokenizers::PaddingStrategy::BatchLongest,
            ..Default::default()
        };
        ...
        let vb = VarBuilder::from_slice_safetensors(weights, DTYPE, &device)
            .map_err(|e| IoError::other(format!("Failed to load weights from buffer: {e}")))?;
        let weights = BertModel::load(vb, &config).map_err(|e| {
            IoError::other(format!(
                "Failed to load BertModel from VarBuilder and config: {e}"
            ))
    ...
    pub fn embedding(&self, input: Vec<&str>) -> io::Result<Vec<Vec<f32>>> {
        ...
        let tokens_ids = encodings
            ...
            .map_err(|e| IoError::other(format!("Failed to create token_ids tensor: {e}")))?;
        let attention_mask = encodings
            .iter()
            .map(|encoding| Tensor::new(encoding.get_attention_mask(), &self.device))
            .collect::<Result<Vec<_>, candle_core::Error>>()
            .map_err(|e| IoError::other(format!("Failed to create attention_mask tensor: {e}")))?;

        let stacked_token_ids = Tensor::stack(&tokens_ids, 0)
            .map_err(|e| IoError::other(format!("Failed to stack token_ids tensor: {e}")))?;
        let attention_mask = Tensor::stack(&attention_mask, 0)
            .map_err(|e| IoError::other(format!("Failed to stack attention_mask tensor: {e}")))?;

        let token_type_ids = stacked_token_ids
            .zeros_like()
            .map_err(|e| IoError::other(format!("Failed to create token_type_ids tensor: {e}")))?;

        let model_output = self
            .weights
            .forward(&stacked_token_ids, &token_type_ids, Some(&attention_mask))
            .map_err(|e| IoError::other(format!("Model forward pass failed: {e}")))?;
        let embeddings = model_output
            .sum(1)
            .map_err(|e| IoError::other(format!("Failed to sum model output: {e}")))?;
        ...
        for i in 0..encodings.len() {
            ...
            let embedding = embedding_i
                .to_vec1()
                .map_err(|e| IoError::other(format!("Failed to convert embedding to Vec<f32>: {e}")))?;
